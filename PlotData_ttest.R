...
## for each AA. Free scales emphasize the differences in each AA
ggplot(data.long, aes(x=response, y=Conc, color=AA)) +
  geom_boxplot() + facet_wrap(~AA, scales = "free")
## With scales = "free" each facet gets its own x-axis labels. this is
## unnecessary because x-axis labels/limits are the same across all plots.
## use "free_y" instead.
ggplot(data.long, aes(x=response, y=Conc, color=AA)) +
...
## return data is a list of data frames, 1 df for each AA.
...
## use lapply (list-apply) to apply the function to all elements of the
## split.data list
lapply(split.data, simpleTTfunc)
...
otherTTfunc <- function(x){
  ...
  s <- tapply(x$Conc, x$response, sd)     # calculate the standard deviation
  # return the results
  c(mean=m[1], sd=s[1],
    mean=m[2], sd=s[2], pval=p)  # you do not need an explicit return() statement in R
}
...
## rbind() to bind all elements to a dataframe
